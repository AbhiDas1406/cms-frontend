...
export const AUTOLOGIN = () => async dispatch => {
  ...
  if (token) {
    ...
    await axios.post("https://cms-backend-phi.vercel.app/auth/verifyToken", { token: token })
      .then((res) => {
        dispatch(LOGIN({
          token: token,
          userId: userId,
...
export const ASYNC_LOGIN = userData => dispatch => {
  ...
  let URL = "https://cms-backend-phi.vercel.app/auth/signin";
...
export const ASYNC_SIGNUP = authData => dispatch => {
  ...
  let URL = "https://cms-backend-phi.vercel.app/auth/signup";
  axios.post(URL, authData)
    .then(response => {
      const token = response.data.token;
      const userId = response.data.userId;
